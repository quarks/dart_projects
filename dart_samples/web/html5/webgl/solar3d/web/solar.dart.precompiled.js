...
function dart_precompiled($collectedClasses) {
  ...
  function Camera(eyePosition, upDirection, lookAtPosition, zNear, zFar, aspectRatio, fOV) {
  ...
  function OrbitPath(_gl, _vertexBuffer, shader, _vertexStride, _vertexCount, _radiusLocation, _originLocation, _colorLocation, _cameraLocation, _positionAttributeIndex) {
    this._gl = _gl;
    this._vertexBuffer = _vertexBuffer;
    this.shader = shader;
    this._vertexStride = _vertexStride;
    this._vertexCount = _vertexCount;
    this._radiusLocation = _radiusLocation;
    this._originLocation = _originLocation;
    this._colorLocation = _colorLocation;
    this._cameraLocation = _cameraLocation;
    this._positionAttributeIndex = _positionAttributeIndex;
  }
  OrbitPath.builtin$cls = "OrbitPath";
  if (!"name" in OrbitPath)
    OrbitPath.name = "OrbitPath";
  $desc = $collectedClasses.OrbitPath;
  if ($desc instanceof Array)
    $desc = $desc[1];
  OrbitPath.prototype = $desc;
  function PlanetShader(shader, gl, cameraTransformLocation, objectTransformLocation, viewTransformLocation, objectScaleLocation, planetIndexLocation, _cameraTransform, _objectTransform, _viewTransform, _objectScale) {
    this.shader = shader;
    this.gl = gl;
    this.cameraTransformLocation = cameraTransformLocation;
    this.objectTransformLocation = objectTransformLocation;
    this.viewTransformLocation = viewTransformLocation;
    this.objectScaleLocation = objectScaleLocation;
    this.planetIndexLocation = planetIndexLocation;
    this._cameraTransform = _cameraTransform;
    this._objectTransform = _objectTransform;
    this._viewTransform = _viewTransform;
    this._objectScale = _objectScale;
  }
  PlanetShader.builtin$cls = "PlanetShader";
  if (!"name" in PlanetShader)
    PlanetShader.name = "PlanetShader";
  $desc = $collectedClasses.PlanetShader;
  if ($desc instanceof Array)
    $desc = $desc[1];
  PlanetShader.prototype = $desc;
  function Shader(vertexShaderSource, fragmentShaderSource, vertexShader, fragmentShader, program) {
  ...
  function Skybox(gl, indexBuffer, vertexBuffer, vertexShader, fragmentShader, program, _cameraTransformLocation, _positionAttributeIndex, _textureAttributeIndex, _vertexCount, _vertexStride, _cameraTransform) {
    this.gl = gl;
    ...
    this.fragmentShader = fragmentShader;
    this.program = program;
    this._cameraTransformLocation = _cameraTransformLocation;
    this._positionAttributeIndex = _positionAttributeIndex;
    this._textureAttributeIndex = _textureAttributeIndex;
    this._vertexCount = _vertexCount;
    this._vertexStride = _vertexStride;
    this._cameraTransform = _cameraTransform;
  }
  Skybox.builtin$cls = "Skybox";
  if (!"name" in Skybox)
    Skybox.name = "Skybox";
  $desc = $collectedClasses.Skybox;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Skybox.prototype = $desc;
  function Solar3DApplication(glContext, canvas, planetShader, textureManager, random, camera, skyBox, controller, solarSystem, orbitPath, ownMouse, keyCodeLeft, keyCodeRight, keyCodeE, keyCodeF, keyCodeJ, keyCodeS, renderTime) {
    ...
    this.keyCodeE = keyCodeE;
    this.keyCodeF = keyCodeF;
    this.keyCodeJ = keyCodeJ;
    this.keyCodeS = keyCodeS;
    this.renderTime = renderTime;
  }
  Solar3DApplication.builtin$cls = "Solar3DApplication";
  if (!"name" in Solar3DApplication)
    Solar3DApplication.name = "Solar3DApplication";
  $desc = $collectedClasses.Solar3DApplication;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Solar3DApplication.prototype = $desc;
  function Solar3DApplication_startup_closure(this_0) {
    this.this_0 = this_0;
  }
  Solar3DApplication_startup_closure.builtin$cls = "Solar3DApplication_startup_closure";
  if (!"name" in Solar3DApplication_startup_closure)
    Solar3DApplication_startup_closure.name = "Solar3DApplication_startup_closure";
  $desc = $collectedClasses.Solar3DApplication_startup_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Solar3DApplication_startup_closure.prototype = $desc;
  function SolarSystem(sun, globalScale, selectedPlanet, planets, lastTime) {
    this.sun = sun;
  ...
  function PlanetaryBody(name, texture, orbitPeriod, solarSystem, height, bodySize, angle, orbitRadius, orbitSpeed, position, planets) {
    this.name = name;
  ...
  function MouseSphereCameraController(zoomVelocity, mouseSensitivity, upAngle, sideAngle, radius, accumDX, accumDY, accumScroll, _origin, _minRadius) {
    this.zoomVelocity = zoomVelocity;
    this.mouseSensitivity = mouseSensitivity;
    this.upAngle = upAngle;
    this.sideAngle = sideAngle;
    this.radius = radius;
    this.accumDX = accumDX;
    this.accumDY = accumDY;
    this.accumScroll = accumScroll;
    this._origin = _origin;
    this._minRadius = _minRadius;
  }
  MouseSphereCameraController.builtin$cls = "MouseSphereCameraController";
  if (!"name" in MouseSphereCameraController)
    MouseSphereCameraController.name = "MouseSphereCameraController";
  $desc = $collectedClasses.MouseSphereCameraController;
  if ($desc instanceof Array)
    $desc = $desc[1];
  MouseSphereCameraController.prototype = $desc;
  function Texture(bindingPoint, texture) {
  ...
  Texture.prototype.get$bindingPoint = function() {
    return this.bindingPoint;
  };
  function TextureManager(gl, baseURL, textures) {
    this.gl = gl;
  ...
  function TextureManager_loadCube_closure(box_1, box_0, this_2, sideTargets_3, img_4, c_5) {
    this.box_1 = box_1;
    this.box_0 = box_0;
    this.this_2 = this_2;
    this.sideTargets_3 = sideTargets_3;
    this.img_4 = img_4;
    this.c_5 = c_5;
  }
  TextureManager_loadCube_closure.builtin$cls = "TextureManager_loadCube_closure";
  if (!"name" in TextureManager_loadCube_closure)
    TextureManager_loadCube_closure.name = "TextureManager_loadCube_closure";
  $desc = $collectedClasses.TextureManager_loadCube_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  TextureManager_loadCube_closure.prototype = $desc;
